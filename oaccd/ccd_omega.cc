...
double Oaccd::ccd_omega_rhf(){

    dpdbuf4 Ints; //Integrals
    ...
    //Amplitudes and integrals should now be in i,a,j,b order
    global_dpd_->buf4_init(&Ints, PSIF_LIBTRANS_DPD, 0, ID("[O,V]"), ID("[O,V]"),
                  ID("[O,V]"), ID("[O,V]"), 0,  "L_pqrs");
    global_dpd_->buf4_init(&Amps, PSIF_CC_TAMPS, 0, ID("[O,V]"), ID("[O,V]"),
                  ID("[O,V]"), ID("[O,V]"), 0,  "T2");

    double cc_energy = global_dpd_->buf4_dot(&Amps, &Ints);

    global_dpd_->buf4_close(&Ints);
